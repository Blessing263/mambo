# Ruzivo — Foundation Spec

> **Ruzivo** (Shona: *"knowledge"*) — a plain-language, citizen-facing information
...
ministry plugs in**. A citizen can ask anything and Ruzivo routes them to the right
ministry, answers from that ministry's official documents, and names the source.

The name scales: *Ruzivo* = "knowledge," not "ICT knowledge."
...
1. **Trust first.** A *wrong* answer is worse than *no* answer. Ruzivo answers only
...
2. **Security allow-list** — Ruzivo only ever crawls/cites official `*.gov.zw`
...
- **Politeness & allow-list** — only official domains, rate-limited, robots-respecting.
  Ruzivo must never look like it's attacking government servers.
