...
Environment (CCE) is the path to running Ruzivo on national, sovereign compute —
...
| **Evaluation pipeline** | Batch, periodic | `scripts/evaluate_ruzivo.py --full`, `validate_corpus.py` on resourced compute |
...
Ruzivo is **cloud-deployed today, not edge.** The CCE roadmap is the documented
