/**
 * Playwright layout verification for Ruzivo.
...
test.describe("Chat flow", () => {
  test("sending a question transitions to conversation view", async ({ page }) => {
    ...
    await expect(page.locator("text=Ruzivo").first()).toBeVisible({ timeout: 15000 });
  });
...
test.describe("API integration", () => {
  ...
  test("asking a question produces an answer with text", async ({ page }) => {
    ...
    await page.keyboard.press("Enter");
    // Wait for streaming answer to appear (the "Ruzivo" label only appears when an answer does)
    await expect(page.locator("text=Ruzivo").first()).toBeVisible({ timeout: 90000 });
