...
This is Mambo's answer to the gap analysis's call for a Zimbabwe citizen-query
benchmark (à la CitizenQuery): citizen-service AI needs measured, not asserted,
behaviour.

## Methodology

Two layers, run by `scripts/evaluate_mambo.py`:
...
```
uv run python scripts/evaluate_mambo.py --full --out submission/data/full_eval.csv
```
